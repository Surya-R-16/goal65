   ...
   return (
      <div className="pb-28 pt-6 px-4 max-w-md mx-auto">
         ...
         <div className="space-y-6">
            ...
            {/* Goals Section */}
            <div className="bg-surface-bright p-6 rounded-[28px] border border-outline/10 space-y-4">
               <h2 className="text-lg font-medium text-onSurface mb-2">Health & Goals</h2>

               {/* BMI Display */}
               <div className="bg-surface-container/50 p-4 rounded-xl flex items-center justify-between">
                  <div>
                     <div className="text-xs font-medium text-onSurface-variant mb-1">Your BMI</div>
                     <div className="flex items-baseline gap-2">
                        <span className="text-2xl font-bold text-primary">
                           {formData.height > 0 ? (formData.weight / ((formData.height / 100) ** 2)).toFixed(1) : '-'}
                        </span>
                        <span className="text-sm font-medium text-onSurface-variant">
                           {(() => {
                              if (formData.height <= 0) return '';
               ...
               <div>
                  <label className="block text-xs font-medium text-onSurface-variant mb-1">Dietary Preference</label>
                  <input
                     value={formData.dietaryPreference || ''}
                     onChange={e => update('dietaryPreference', e.target.value)}
                     placeholder="e.g. Vegetarian, Keto, South Indian..."
                     className="w-full bg-surface-container rounded-xl p-3 focus:outline-primary"
                  />
               </div>
